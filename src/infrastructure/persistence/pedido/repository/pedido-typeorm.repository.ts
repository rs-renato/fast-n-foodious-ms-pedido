...
export class PedidoTypeormRepository implements IPedidoRepository {
  ...
  async find(options: any): Promise<Pedido[]> {
    ...
    return this.repository
      .find(options)
      .then((pedidoEntities) => {
        this.logger.debug(`Consulta de pedidos realizada com sucesso com as condições: '${JSON.stringify(options)}'`);
  ...
  async findBy(attributes: any): Promise<Pedido[]> {
    ...
    return this.repository
      ...
      .catch((error) => {
        throw new RepositoryException(
          `Houve um erro ao buscar os pedidos com os parâmetros: '${JSON.stringify(attributes)}': ${error.message}`,
        );
      });
  }
