...
export class PedidoPagamentoPendenteValidator implements IValidator<ItemPedido> {
    ...
    async validate(itemPedido: ItemPedido): Promise<boolean> {
        ...
        this.logger.log(`Inicializando validação ${PedidoPagamentoPendenteValidator.name} de pagamento pendente. itemPedido: ${JSON.stringify(itemPedido)}`);
        ...
        await this.repository.findBy({ id: itemPedido.pedidoId })
