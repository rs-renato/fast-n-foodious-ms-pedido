...
export class SesIntegration {
  ...
  constructor(
    ...
    private buscarClientePorIdPedidoUsecase: BuscarClientePorIdPedidoUsecase,
  ) {}

    async sendEmailPagamento(pagamento: PagamentoDto): Promise<SendMessageCommandOutput> {

      return await this.buscarClientePorIdPedidoUsecase.buscarClientePorPedidoId(pagamento.pedidoId)
        .then(async(cliente) => {
          ...
          const command = new SendEmailCommand({
            ...
            Message: {
              Subject: {
                Charset: "UTF-8",
                Data: (pagamento.estadoPagamento === EstadoPagamento.CONFIRMADO ? this.SUBJECT_PAGAMENTO_APROVADO : this.SUBJECT_PAGAMENTO_RECUSADO) + `#${pagamento.pedidoId}`,
              },
              Body: {
                Text: {
                  Charset: "UTF-8",
                  Data: pagamento.estadoPagamento === EstadoPagamento.CONFIRMADO
          ...
          return await this.sesClient
            ...
            .catch((error) => {
              this.logger.error(
                `Erro ao enciar email para notificação de pagamento recusado: ${JSON.stringify(error)} - Command: ${JSON.stringify(command)}`,
              );
